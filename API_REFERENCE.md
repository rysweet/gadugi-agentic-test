...
The CLIAgent recognizes these step actions for command execution:

| Action | Description |
|---|---|
| `execute` | Execute a command step |
| `run` | Run a command and validate its output |
| `command` | Execute a command (used by legacy format conversion) |
| `execute_command` | Execute a shell command |
| `multi_command` | Execute multiple commands in sequence |
| `execute_with_retry` | Retry a command until success or limit |
| `background_process` | Start a long-running command |

The first four (`execute`, `run`, `command`, `execute_command`) are all command-execution actions. The `command` action is the one generated by the legacy format converter when processing `type: command` steps.

When a step has a `target` field, the CLIAgent uses it as the command string directly. Otherwise, it extracts the command from `params.command` or `params.input`.
...
### TestStep (scenarios module)

The `TestStep` interface used by `ScenarioDefinition` (from `src/scenarios`):
...
The `target` field carries the command string directly for CLI steps converted from legacy format (`type: command` → `action: 'command'`). The `expected` field carries the expected output for post-execution validation. Both are optional and only populated by the legacy format converter when the source YAML provides them.

### OrchestratorStep

The `OrchestratorStep` interface used by `OrchestratorScenario` (from `src/models/TestModels`). The deprecated alias `TestStep` is re-exported for backward compatibility but will be removed in v2.0:
...
/** @deprecated Use OrchestratorStep — will be removed in v2.0 */
type TestStep = OrchestratorStep;
```
...
export class ScenarioLoader {
   ...
   * Always returns an array — canonical/wrapped formats return [scenario],
   * legacy format (scenarios: [...]) returns all scenarios from the array.
   */
  static async loadFromFile(path: string): Promise<ScenarioDefinition[]>;

  /**
   * Load all scenarios from a directory. Results from all files are flattened
   * into a single array — a file with 10 legacy scenarios contributes 10 entries.
   */
  static async loadFromDirectory(directory: string): Promise<ScenarioDefinition[]>;
}
```

**Supported YAML formats:**

| Format | Detection | Return |
|---|---|---|
| Canonical | Top-level `name` + `steps` + `agents` | `[scenario]` |
| Wrapped | `scenario: { name, steps, agents, ... }` | `[scenario]` |
| Legacy (multi-scenario) | `scenarios: [...]` array | All scenarios |

**Legacy format conversion:** When the top-level `type` field is `'cli'`, each scenario in the array gets a `cli-agent` (type `'cli'`). Steps with `type: command` are mapped to `action: 'command'` with the command string set as `target` and `expected_output` set as `expected`.

**Examples:**
```typescript
// Load single file — may return 1 or many scenarios
const scenarios = await ScenarioLoader.loadFromFile('./tests/login.yaml');
for (const scenario of scenarios) {
  console.log(`Loaded: ${scenario.name}`);
}

// Load all scenarios from directory (flattened)
const allScenarios = await ScenarioLoader.loadFromDirectory('./tests/');

// Each scenario is a full ScenarioDefinition
allScenarios.forEach(s => {
  console.log(`${s.name}: ${s.steps.length} steps, agent: ${s.agents[0].type}`);
});
```
