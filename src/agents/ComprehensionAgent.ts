...
import { OrchestratorScenario, OrchestratorStep, VerificationStep, Priority, TestInterface } from '../models/TestModels';
...
Extract and return ONLY valid JSON in this exact format:
{
   ...
   */
  private generateSuccessSteps(spec: FeatureSpec): OrchestratorStep[] {
   ...
   * @returns Array of test steps
   */
  private generateFailureSteps(spec: FeatureSpec, failureMode: string): OrchestratorStep[] {
    return [
      {
        action: 'execute',
        target: spec.name.toLowerCase().replace(/\s+/g, '-'),
   ...
   * @returns Array of test steps
   */
  private generateEdgeCaseSteps(spec: FeatureSpec, edgeCase: string): OrchestratorStep[] {
    return [
      {
        action: 'execute',
        target: spec.name.toLowerCase().replace(/\s+/g, '-'),
