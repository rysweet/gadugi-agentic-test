...
exports.capturePageScreenshot = capturePageScreenshot;
...
class ScreenshotManager {
     ...
     */
    async capturePageScreenshot(page, options = {}) {
     ...
     */
    async captureSequence(page, count, interval, options = {}) {
        const screenshots = [];
     ...
     */
    async captureComparison(page, action, options = {}) {
        // Capture before
        const beforeOptions = {
            ...options,
     ...
     */
    async compareScreenshots(baselineImage, actualImage, options = {}) {
        const { threshold = 0.1, algorithm = 'pixel-by-pixel', includeAA = false, createDiffImage = true, colorOptions = {} } = options;
        try {
            // Load both images using Jimp
            const [baseline, actual] = await Promise.all([
                jimp_1.Jimp.read(baselineImage),
                jimp_1.Jimp.read(actualImage)
            ]);
            ...
            const differentPixels = (0, pixelmatch_1.default)(baselineBuffer, actualBuffer, diffBuffer, width, height, pixelmatchOptions);
            ...
            if (createDiffImage && differentPixels > 0) {
                ...
                diffImagePath = await this.createDiff(baselineImage, actualImage, path.join(path.dirname(baselineImage), `diff_${Date.now()}.png`), {
                    algorithm,
                    colorOptions,
                    includeAA
                });
     ...
     */
    async createDifferenceImage(baselineImage, actualImage, outputPath) {
     ...
     */
    async createDiff(baselineImage, actualImage, outputPath, options = {}) {
        ...
        try {
            ...
            if (showSideBySide) {
                ...
                const sideBySideImage = new jimp_1.Jimp({ width: sideBySideWidth, height: targetHeight, color: 0xFFFFFFFF });
                ...
                const font = await (0, jimp_1.loadFont)(fontPath);
            ...
            // Save the diff image
            await diffImage.write(outputPath);
            return outputPath;
        }
        catch (error) {
     ...
     */
    async createPixelDiff(baseline, actual, colorOptions, includeAA) {
        const width = baseline.width;
        const height = baseline.height;
        const { removedColor = [255, 0, 0], // Red for removed
        addedColor = [0, 255, 0], // Green for added
        changedColor = [255, 255, 0], // Yellow for changed
        alpha = 255 } = colorOptions;
        // Convert to RGBA buffers
        const baselineBuffer = new Uint8ClampedArray(baseline.bitmap.data);
        const actualBuffer = new Uint8ClampedArray(actual.bitmap.data);
        const diffBuffer = new Uint8ClampedArray(width * height * 4);
        // Use pixelmatch for accurate pixel comparison
        const differentPixels = (0, pixelmatch_1.default)(baselineBuffer, actualBuffer, diffBuffer, width, height, {
            threshold: 0.1,
            includeAA,
            alpha: 1.0,
        ...
        const diffImage = new jimp_1.Jimp({ width, height });
        for (let i = 0; i < width * height; i++) {
            ...
            const color = (0, jimp_1.rgbaToInt)(finalColor[0], finalColor[1], finalColor[2], finalColor[3]);
     ...
     */
    async createPerceptualDiff(baseline, actual, colorOptions) {
        const width = baseline.width;
        const height = baseline.height;
        const diffImage = new jimp_1.Jimp({ width, height });
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                const baselineColor = (0, jimp_1.intToRGBA)(baseline.getPixelColor(x, y));
                const actualColor = (0, jimp_1.intToRGBA)(actual.getPixelColor(x, y));
                ...
                const colorDiff = Math.sqrt(Math.pow(baselineColor.r - actualColor.r, 2) +
     ...
     */
    async createStructuralDiff(baseline, actual, colorOptions) {
        const width = baseline.width;
        const height = baseline.height;
        const { removedColor = [255, 0, 0], addedColor = [0, 255, 0], changedColor = [255, 255, 0], alpha = 255 } = colorOptions;
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                ...
                const baselineGray = (0, jimp_1.intToRGBA)(baselineEdge);
                const actualGray = (0, jimp_1.intToRGBA)(actualEdge);
                ...
                if (edgeDiff > 50) {
                    ...
                    const originalColor = (0, jimp_1.intToRGBA)(actual.getPixelColor(x, y));
            ...
            for (let x = 1; x < width - 1; x++) {
                ...
                for (let ky = -1; ky <= 1; ky++) {
                    for (let kx = -1; kx <= 1; kx++) {
                        const pixel = (0, jimp_1.intToRGBA)(image.getPixelColor(x + kx, y + ky));
                ...
                const color = (0, jimp_1.rgbaToInt)(clampedMagnitude, clampedMagnitude, clampedMagnitude, 255);
     ...
     */
    getScreenshotsByScenario(scenarioId) {
        return Array.from(this.metadata.values()).filter(meta => meta.scenarioId === scenarioId);
    }
    /**
     * Get all screenshots for a specific step
     */
    getScreenshotsByStep(scenarioId, stepIndex) {
        return Array.from(this.metadata.values()).filter(meta => meta.scenarioId === scenarioId && meta.stepIndex === stepIndex);
    }
    /**
     ...
     */
    async archiveRun(archivePath) {
        const finalArchivePath = archivePath || path.join(this.organizationOptions.baseDir, 'archives', `${this.runId}.tar.gz`);
        ...
        await fs.writeFile(finalArchivePath.replace('.tar.gz', '.json'), JSON.stringify(manifest, null, 2));
        return finalArchivePath;
    }
    /**
     * Generate a unique filename
     */
    generateFilePath(type, scenarioId, stepIndex) {
     ...
     */
    async exportMetadata(filePath) {
        const finalFilePath = filePath || path.join(this.organizationOptions.baseDir, `${this.runId}_metadata.json`);
     ...
     */
    async batchCompareScreenshots(comparisons) {
        const results = [];
        for (const comparison of comparisons) {
            try {
                const result = await this.compareScreenshots(comparison.baseline, comparison.actual, comparison.options);
     ...
     */
    async calculateSimilarityScore(baseline, actual, options = {}) {
        ...
        const overall = pixelResult.similarityPercentage * weights.pixel +
    ...
    async generateComparisonReport(results, outputPath) {
        const reportPath = outputPath || path.join(this.organizationOptions.baseDir, `comparison_report_${this.runId}.json`);
        const report = {
            runId: this.runId,
            timestamp: new Date(),
            summary: {
 ...
 */
function createScreenshotManager(options) {
    return new ScreenshotManager(options);
}
/**
 * Convenience function for quick page screenshot
 */
async function capturePageScreenshot(page, filePath, options) {
 ...
 */
async function captureElementScreenshot(element, filePath, options) {
    const finalOptions = { ...DEFAULT_OPTIONS, ...options, path: filePath };
