...
program
    ...
    try {
        ...
        try {
            ...
            const scenarioFiles = scenarios.map((s) => path.join(options.directory, `${s.name.toLowerCase().replace(/\s+/g, '-')}.yaml`));
...
program
    ...
    try {
        ...
        logInfo(`Watching directory: ${chalk_1.default.cyan(options.directory)}`);
        ...
        const runTests = async (changedFile) => {
            if (changedFile) {
                logInfo(`File changed: ${chalk_1.default.yellow(changedFile)}`);
            }
            try {
        ...
        watcher.on('add', (filePath) => {
            logInfo(`New file added: ${chalk_1.default.green(filePath)}`);
            if (timeout)
                clearTimeout(timeout);
            timeout = setTimeout(() => runTests(filePath), debounceTime);
        });
        watcher.on('unlink', (filePath) => {
            logInfo(`File deleted: ${chalk_1.default.red(filePath)}`);
            if (timeout)
                clearTimeout(timeout);
        ...
        if (options.json) {
            ...
            filteredScenarios.forEach((scenario, index) => {
                ...
                if (scenario.metadata?.tags && scenario.metadata.tags.length > 0) {
                    const tagStr = scenario.metadata.tags.map(tag => chalk_1.default.cyan(`#${tag}`)).join(' ');
...
program
    ...
    .action(async (options) => {
    try {
        logInfo(`Initializing new testing project in: ${chalk_1.default.cyan(options.directory)}`);
        ...
        if (directoryExists && !options.force) {
            ...
            const files = await fs.readdir(options.directory);
            if (files.length > 0) {
                throw new CLIError(`Directory ${options.directory} is not empty. Use --force to overwrite existing files.`, 'DIRECTORY_NOT_EMPTY');
            }
        }
        ...
        console.log(chalk_1.default.gray('5.'), 'Run tests with: gadugi-test run');
        console.log();
    }
    catch (error) {
        if (error instanceof CLIError) {
...
program
    ...
    if (command) {
        ...
        console.log();
        console.log(chalk_1.default.bold('Gadugi Agentic Test - Examples:'));
        console.log();
...
program
    ...
    console.log();
    console.log('Run', chalk_1.default.cyan('gadugi-test help'), 'for usage information.');
    process.exit(1);
});
